document.addEventListener('DOMContentLoaded', () => {
    ...
    taskForm.onsubmit = (event) => {
        ...
        const validation = validateTaskForm(task.title, task.category, task.dueDate);
    ...
    editTaskForm.onsubmit = (event) => {
        ...
        const updatedTask = {
            ...
            category: editTaskForm.editCategory.value
        };

        const validation = validateTaskForm(updatedTask.title, updatedTask.category, updatedTask.dueDate);
        if (!validation.isValid) {
            alert(validation.errorMessage);
            return;
        }
    ...
    function validateTaskForm(title, category, dueDate) {
        let isValid = true;
        let errorMessage = '';
        ...
        if (!dueDate) {
            errorMessage += 'A data de vencimento é obrigatória.\n';
            isValid = false;
        }
...
function getTasksNearDeadline() {
    ...
    return tasks.filter(task => {
        ...
        return taskDueDate <= twoDaysAhead && task.status !== 'concluída';
    });
}
